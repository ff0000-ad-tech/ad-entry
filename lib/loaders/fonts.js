...
export const loadFont = async path => {
	...
	if (document.fonts && window.FontFace && !_hasSafari10Bug()) {
		const font = new FontFace(asset.id, `url(${asset.src})`)
		await font.load()
		document.fonts.add(font)
		fontLoadPromise = Promise.resolve(font)
	} else {
		let format = 'truetype'
		if (asset.fileType.match(/woff/)) {
			format = 'woff'
		}
		const assembledFontRule = "@font-face { font-family: '" + asset.id + "'; src: url(" + asset.src + ") format('" + format + "'); }"
